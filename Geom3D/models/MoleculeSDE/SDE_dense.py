...
class SDE(abc.ABC):
    ...
    def reverse(self, score_fn, probability_flow=False):
        ...
        class RSDE(self.__class__):
            ...
            def sde(self, x, adj, flags, t, representation, SDE_model, is_adj):
                ...
                score = score_fn(perturbed_x, adj, flags, t)
                drift = drift - diffusion[:, None, None] ** 2 * score * (0.5 if self.probability_flow else 1.)
                # -------- Set the diffusion function to zero for ODEs. --------
                diffusion = 0. if self.probability_flow else diffusion
                return drift, diffusion

            def discretize(self, x, adj, flags, t, representation, SDE_model, is_adj):
                """Create discretized iteration rules for the reverse diffusion sampler."""
                perturbed_x = SDE_model.embedding_3D(representation) + SDE_model.embedding_X(x)  # [B, max_num_nodes, hdim]

                f, G = discretize_fn(adj, t) if is_adj else discretize_fn(x, t)  # [B, max_num_nodes, max_num_nodes]
                score = score_fn(perturbed_x, adj, flags, t)
